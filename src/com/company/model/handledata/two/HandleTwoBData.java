package com.company.model.handledata.two;
...
public class HandleTwoBData extends HandleData {


    public HandleTwoBData(DataReader dataOneReader) {
        super(dataOneReader);
    }
    ...
    /**
     * 1、如果“状态”为“新建”，生成联通接单催办表
     * 2、在联通接单催办表中，根据“告警发生地市”，
     * 通过微信发送给不同处理人（微信发送模块可以设置不同地市接收人或指定群内根据所属地市判断后@不同人员微信）
     *
     * 3、如果“状态”为“处理中”且“系统记录故障恢复时间”有值，生成联通回单催办表；
     * 4、在联通通回单催办表中，根据“告警发生地市”判断，通过微信发送给不同处理人，
     * 微信发送模块可以设置不同地市接收人或指定群内根据所属地市判断后@不同人员微信）
     *
     * 5、如果如果“状态”为“处理中”且“系统记录故障恢复时间”为空
     * 6、计算当前时间减“创建时间”的差；
     * 7、加可设置筛选条件（分钟），得出结果表-联通处理催办表；
     * 8、通过联通催办表中的“告警发生地市”判断，
     * 然后通过微信发送给不同处理人（微信发送模块可以设置不同地市接收人或指定群内根据所属地市判断后@不同人员微信）
     */
    @Override
